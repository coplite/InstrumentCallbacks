...
int main()
{
	HANDLE handle = CreateFileMappingW(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, 256, L"InfoSyscall");
	LPVOID address = MapViewOfFile(handle, FILE_MAP_ALL_ACCESS, 0, 0, 0);
	HANDLE event = CreateEventW(NULL, FALSE, FALSE, L"InfoSyscall");

	const char* dllpath = "C:\\Users\\Owner\\Desktop\\Projects\\InstrumentCallbacks\\payload.dll";
	const char* exe = "target.exe";
